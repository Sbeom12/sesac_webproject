...
def user_signup():
    ...
    if(request.method == 'POST'):
        ...
        if(tel==newTel):
            ...
            return render_template('pages/user.signup.html', data="userId")
        elif(subNm==newSubNm):
            print("이미 존재하는 subNm 입니다.")
            flash("이미 존재하는 subNm 입니다. 다시 입력해주세요.")
            return render_template('pages/user.signup.html', data="subNm")
...
def user_mypage():    
    ...
    if 'userId' in session:
        return render_template('pages/user.mypage.html', data='data')
    else:
